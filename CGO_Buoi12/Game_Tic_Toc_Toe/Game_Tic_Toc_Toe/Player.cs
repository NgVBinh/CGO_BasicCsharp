...
namespace Game_Tic_Toc_Toe
{
    ...
    public class Robot : Player
    {
        ...
        public Robot(char Sign = '0'):base(Sign) {
            lstNumber =new List<int>() { 1,2,3,4,5,6,7,8,9};
        }

        public override int takeTurn()
        {
            int index= random.Next(0,lstNumber.Count);
            return lstNumber[index];
         
        }

        public bool removeIndexByValue(int value)
        {
            int index=lstNumber.IndexOf(value);
            if(index<0) return false;
            lstNumber.RemoveAt(index);
            return true;
        }
    }
}